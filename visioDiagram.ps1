...
foreach ($cluster in $clusters)
{
    ...
    foreach($hostvm in $hosts)
    {
        ...
        $x++
        $vms = Get-VMHost -Name $hostvm.Name | get-vm| ?{$_.powerstate -eq "PoweredOn" -and $_.guest -like "*linux*"}
        foreach($vm in $vms)
        {
            $shapevm = Create-visioShape -master $MasterVM -points $x,$y -name $vm.Name
            $txt = $vm.name +"`n"+ $vm.Guest.OSFullName + "`n"+  $vm.NumCpu   + "vCPU`n"+ $vm.MemoryGB + "GB`nIP:"+$vm.Guest.IPAddress[0]
            Set-VisioShapeText -Text $txt -Shapes (Get-VisioShape -NameOrID $shapevm.Name)
            Create-VisioConnection -FromName $hostvm.Name -toName $vm.Name
            $x++
