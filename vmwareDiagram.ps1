...
$stnPath = [system.Environment]::GetFolderPath('MyDocuments') + "\"  
...
#DRAW ALL Physical VMHOST NODES with Node Name, Total Memory and vCenter version and connect them to the cluster object
    Foreach ($node in $allNODES) {  
            ...
            $x = 2  
            $y += 1.5  
            $nodeInfo = "NODE: " + $node.Name  +  "`nServer Model: "+$node.Manufacturer+ " " +$node.Model+ "`nESXIVersion: " + $node.Version + "`nTotalMemory(GB): " + [math]::Round([decimal]$node.MemoryTotalGB,2) +"`nCPU Model: "  + $node.ProcessorType + "`nTotal MHz: "  +$node.CpuTotalMhz+ "`nNum CPU: " +$node.NumCpu 
...
#$DocObj.SaveAs("C:\Users\m41718\Documents\$cluster.vsd")
