...
##Importação de Usuários e Definição de OU
$usuarios = Get-Content -Path c:\temp\usuarios.txt
$OU = "OU=Ativos,OU=Usuários,DC=Trescinco,DC=local"
...
$NovoUsuario = New-ADUser -Name "$PrimeiroNome $UltimoNome" -GivenName $PrimeiroNome -Surname $UltimoNome -UserPrincipalName "$Login@trescinco.local" -Title "$Cargo" -SamAccountName "$Login" -Path $OU -Enabled $True -AccountPassword (ConvertTo-SecureString -AsPlainText "$password" -Force) -PassThru -ChangePasswordAtLogon $True
